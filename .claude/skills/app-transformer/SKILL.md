---
name: app-transformer
description: "Deterministic pipeline that transforms any mid-tier, generic, template-looking, or vibe-coded app into a super-premium product — regardless of which LLM runs it. Use whenever the user says 'transform', 'upgrade', 'make it premium', 'make it look expensive', 'redesign', 'polish this app', 'it looks AI-generated', 'make it high tier', or wants a full-app quality pass on a web app, dashboard, SaaS, landing page, mobile-style web app, internal tool, or portfolio. Also use when starting a new app that must be top-tier from day one. Works phase-by-phase with copy-paste values so even lower-capability models produce the same result."
---
