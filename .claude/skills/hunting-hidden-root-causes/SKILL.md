---
name: hunting-hidden-root-causes
description: Use when a symptom is reported using the vocabulary of one component ("payload received," "deploy shipped," "config updated," "access revoked") but a downstream system doesn't reflect it — any "A happened, B didn't follow" bug, "works here but not there," or a bug that survives repeated debugging sessions where the same fix keeps getting proposed and doesn't stick. Domain-agnostic: web apps, CI/CD, message queues, config distribution, permissions, data pipelines — anywhere a write/event must propagate through independent systems.
---
